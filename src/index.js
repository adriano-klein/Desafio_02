...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro === false && user.todos.length === 10) {
    return response
      .status(403)
      .json({ error: "You can only continue with a pro plan" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!isUuid) {
    return response.status(400).json({ error: "Task id is not valid" });
  }
...
function findUserById(request, response, next) {
  ...
  if (!isUuid) {
    return response.status(404).json({ error: "User id is not valid" });
  }
